...
In a first step, I am comparing China, South Korea, Italy, and Germany. Why these countries? These countries are at very different stages or show very different developments (in terms of size and patterns). China, for example, was the first to experience the outbreak and they seem to have contained the spreading of the virus by now. South Korea is similarly controlling the virus outbreak (the pattern follows China). Italy has been experiencing one of the worst outbreaks and drastic measures have been taken, we see the first signs that it slowly gets better. The virus has reached Germany considerably later. Although the growth rate was very steep in the beginning, it now starts to follow the pattern of China and South Korea. 
...
europe <- c("Italy", "Spain", "Germany", "France", 
            ...
            "Serbia", "Luxembourg", "Finland", "Ukraine")
...
```{r europe_plot, fig.height=7, fig.width = 9}
data %>%
  filter(confirmed >= 100) %>%
  filter(country %in% europe) %>%
  group_by(country) %>%
...
america <- c("US", "Canada", "Brazil", "Chile",
             ...
             "Panama", "Dominican Republic", "Colombia")
...
```{r northamerica, fig.height=6, fig.width = 9}
data %>%
  filter(confirmed >= 100) %>%
  filter(country %in% america) %>%
  group_by(country) %>%
...
```{r middleeast, fig.height=5, fig.width = 8}
data %>%
  filter(confirmed >=100) %>%
  filter(country %in% middleeast) %>%
  group_by(country) %>%
...
```{r,results = 'asis'}
africa <- c("South Africa", "Egypt", "Algeria", "Morocco")
...
```{r africa, fig.height=7, fig.width = 8}
data %>%
  filter(confirmed >= 100) %>%
  filter(country %in% africa) %>%
  group_by(country) %>%
...
One thing that is constantly debatted is how to visualize growth (of total confirmed cases) at all. Log-transform the y-axis or not? Plot against the date? Plot against days after 100th case? Plot something entirely different?
...
pg1 <- data %>%
  ...
  geom_line(data = . %>% filter(country == "Korea, South"), 
            aes(x = days, y = confirmed), colour="#ffcc00", size=.8) +
  geom_dl(data = . %>% filter(country == "Korea, South"), 
  ...
  labs(x = "days after 100 cases", y = "total cases (limited to 500.000)", color = "")
